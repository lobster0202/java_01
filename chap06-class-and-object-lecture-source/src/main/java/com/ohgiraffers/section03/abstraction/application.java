...
public class application {
    public static void main(String[] args) {
        /* 추상화란?
        * 공통된 부분을추출하고 공통되지 않은 부분을 제거한다는 의미,
        * 추상화의 목적은 유연성을 확복하기 위함이다.
        ...
        * 오류 발생 가능성을 감소시키고 유지보수성을 증가시킨다. */
        ...
        /* 시스템 요구사항
        * 1. 운전자는 시동걸기, 엑셀 밞기, 브레이크 밞기, 시동 끄기를 할 수 있다.
        * 2. 자동차는 시동걸기, 앞으로 가기, 멈추기, 시동끄기를 할 수 있다.
        * 3. 자동차는 처음에 멈춘 상태로 대기하고 있는다. (시동이 꺼져있다.)
        * 4. 운전자는 먼저 자동차에 시동을 건다. 이미 걸려있는 경우 다시 시동을 걸 수 없다.
        * 5. 운전자가 엑셀을 밞으면 시동이 걸린 상태일 경우 자동차는 시속이 10km/h 증가하며 앞으로 나간다.
        * 6. 자동차가 달리는 중인 경우 브레이크를 밞으면 자동차의 시속은 0으로 떨어지며 멈춘다.
        * 7. 브레이크를 밞을 때 자동차가 달리는 중이 아니라면 이미 멈춰있는 상태라고 안내한다.
        ...
        *    1) CarDriver 클래스
        *           -속성 : 자동차(Car 클래스)
        *           -행위 : 시동을 걸어라, 엑셀 밞아라, 브레이크 밞아라, 시동을 꺼라
        *    2) car 클래스
        *           -속성 : 시동 상태, 현재 시속
...
//      label:
        while (true) {
            ...
            switch (no) {
                ...
                case 9 : System.out.println("프로그램을 종요합니다."); break; //label;
                default:
            ...
            /* while문에 label을 붙이고 case9 뒷 부분까지 label을 붙여서 브레이크를 걸어줘서
               while문을 빠져 나가는 방법도 있다.*/
        }
    }
}
