...
public class application1 {
    public static void main(String[] args) {
         ...
         * 그러므로 하나의 타입으로 여러 타입의 인스턴스를 처리 할 수 있기도 하고,
        ...
        * Tiger는 Tiger 타입이기도하면서 Animal 타입이기도 하다. */

        /* 다형성 -> 부모 타입으로 자식 인스턴스 주소값 저장 */
        Animal a1 = new Cat();                              // 자료형(타입이라고 생각 가능) / 레퍼런스 변수 =
        Animal a2 = new Tiger();
...
//        a1.jump();        // (석현)타입이 animal 타입이라서 고양이만 가지고 있는 고유한 동작은 실행하지 못함
//        a2.bite();
...
//        ((Tiger)a1).bite();             //  고양이는 호랑이가 될 수 없다.
        ...
        * down-casting : 하위 타입으로 형변환 */
        ...
        Animal animal1 = (Animal)new Cat();         // up-casting (수동) 명시적 형변환
        Animal animal2 = new Cat();                 // up-casting (자동) 묵시적 형변환

        Cat cat1 = (Cat)animal1;                    // down-casting (수동) 명시적 형변환
//        Cat cat2 = animal2;                       // down-casting (자동) 묵시적 형변환 안됨
                                                    // (석현) 변수에서 큰 값에서 작은값으로 형변환 할 때
                                                    // 자동으로 변환 안되는거랑 똑같은 상황임
